...
class DocBase extends Ctrl {
  ...
  async removeMany() {
    ...
    if (typeof transforms === 'string' && transforms.length !== 0) {
      ...
      if (fs.existsSync(process.cwd() + "/config/plugins.js")) {
        ...
        let removeTransformDoc = _.get(plugins, 'document.transforms.removeMany')
        if (removeTransformDoc && Array.isArray(removeTransformDoc)) {
          ...
          if (typeof notRemoveDatas === "string" && notRemoveDatas === "ALL") return new ResultFault("删除失败！不满足所选插件的要求")
  ...
  async cutDocs(oldDb, oldCl, newDb, newCl, docIds = null, options = {}, oldDocus = null) {
    ...
    if (newDocs.length == 0) return [false, "没有选择数据或为重复数据或不满足插件要求"]
  ...
  async getDocsByRule2(dbName, clName, rules, planTotalColumn = "need_sum") {
    ...
    let docs = rules.map(async rule => {
      ...
      for (const schemaKey in rule) {
        if (schemaKey === planTotalColumn || schemaKey === "_id" || schemaKey === TMSCONFIG["TMS_APP_DEFAULT_UPDATETIME"] || schemaKey === TMSCONFIG["TMS_APP_DEFAULT_CREATETIME"]) continue
